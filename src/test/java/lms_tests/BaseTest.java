...
public class BaseTest {
    public boolean ADD_FILES_TO_ALLURE_REPORT = !true; // * true - добавляются файлы в ALLURE-report только для упавших тестов
    ...
    public void attachFilesToFailedTest(ITestResult result) throws IOException {
        ...
        if (ADD_PAGE_SOURCE_TO_REPORT && !result.isSuccess()) {
            try { String pageSource = page.content();
...
//        Allure.step("Attach files to Allure report", () -> {
//            String TEST_NAME = result.getMethod().getMethodName();
...
//    }
    ...
    public void tearDown() {
        Allure.step("Closing the browser after all tests in the class are executed", () -> {
            if (browser != null) {
                browser.close();
                browser = null;
            }
    ...
    // Собственная аннотация для параметризации тестов
